...
We evaluated three inversion methods on synthetic datasets generated from Neural Ordinary Differential Equations (Neural ODEs) and Neural Operators (FNO): the Simple Harmonic Oscillator (SHO), the Damped Harmonic Oscillator (DHO), Bessel equations ($n=0,1$), the Cauchy-Euler equation ($a=1,b=4$), and the 1D Advection equation.

The bounds derived from point-wise inverse FFT (red dashed lines), Interval FFT set propagation (grey shaded region), and perturbation sampling (blue dash-dot envelopes) are visualized in Figure~\ref{fig:sho}, Figure~\ref{fig:dho}, and Figure~\ref{fig:advection}. To make the guaranteed interval envelopes explicit, we also include standalone interval plots for SHO and DHO in Figure~\ref{fig:sho-interval-only} and Figure~\ref{fig:dho-interval-only}. We additionally report empirical coverage curves for all inversion methods on SHO and DHO in Figure~\ref{fig:sho-coverage} and Figure~\ref{fig:dho-coverage}.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{images/bessel_n0_bounds_comparison.png}
    \caption{Bessel $n=0$: physical bounds via point-wise inversion, Interval FFT set propagation, and perturbation sampling.}
    \label{fig:bessel-n0}
\end{figure}

\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{images/bessel_n1_bounds_comparison.png}
    \caption{Bessel $n=1$: physical bounds via point-wise inversion, Interval FFT set propagation, and perturbation sampling.}
    \label{fig:bessel-n1}
\end{figure}

\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{images/cauchy_euler_a1_b4_bounds_comparison.png}
    \caption{Cauchy-Euler ($a=1,b=4$): physical bounds via point-wise inversion, Interval FFT set propagation, and perturbation sampling.}
    \label{fig:cauchy-euler}
\end{figure}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{images/advection_bounds_comparison.png}
    \caption{Physical bounds via PRE inversion for the 1D Advection equation ($U_t + v U_x = 0$). For a fixed-time spatial slice, point-wise (red), Interval FFT (grey), and perturbation sampling (blue) are shown together. To keep bounds numerically reasonable, envelope widths are clipped using a robust signal-scale cap.}
    \label{fig:advection}
\end{figure}
...
\begin{table}[h]
    ...
    \begin{tabular}{lccc}
        ...
        SHO & 0.00 & 1.00 & 1.00 \\
        DHO & 0.00 & 1.00 & 1.00 \\
        Bessel $n=0$ & 1.00 & 1.00 & 0.00 \\
        Bessel $n=1$ & 1.00 & 1.00 & 0.40 \\
        Cauchy-Euler $(a=1,b=4)$ & 1.00 & 1.00 & 0.00 \\
        Advection (mid-time slice) & 1.00 & 1.00 & 0.00 \\
        \bottomrule
    \end{tabular}
\end{table}
